...
native_prefix = II if sys.byteorder == "little" else MM
#
# --------------------------------------------------------------------
# Read TIFF files
...
class ImageFileDirectory(collections.MutableMapping):
    ...
    def load_short(self, data):
        l = [self.i16(data, i) for i in range(0, len(data), 2)]
        return tuple(l)
    load_dispatch[3] = (2, load_short)

    def load_long(self, data):
        l = [self.i32(data, i) for i in range(0, len(data), 4)]
        return tuple(l)
    load_dispatch[4] = (4, load_long)

    def load_rational(self, data):
        l = [(self.i32(data, i), self.i32(data, i+4)) for i in range(0, len(data), 8)]
        return tuple(l)
